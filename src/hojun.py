...
class lane_detect:
	...
	def image_CB(self,data):
		try:
			...
			lines = self.find_lanes(frame)
			bird_view = self.bird_eye_view(frame, height, width)


			# cv2.imshow("Hough_lane",bird_view)
	...
	def find_lanes(self,image):
	    ...
	    # 허프 변환을 통한 직선 검출
		lines = cv2.HoughLinesP(masked_edges, rho=1, theta=np.pi/180, threshold=100, minLineLength=100, maxLineGap=100)
